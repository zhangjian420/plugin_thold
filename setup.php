...
function plugin_thold_install($upgrade = false) {
	...
	// Setup permissions
	api_plugin_register_realm($plugin, 'thold.php', '阈值', 1);
	api_plugin_register_realm($plugin, 'thold_templates.php', '阈值模板', 1);
	api_plugin_register_realm($plugin, 'notify_lists.php', '阈值通知列表', 1);
	api_plugin_register_realm($plugin, 'thold_graph.php,graph_thold.php,thold_view_failures.php,thold_view_normal.php,thold_view_recover.php,thold_view_recent.php,thold_view_host.php', '查看阈值', 1);
	...
	if ($action == 'plugin_thold_create') {
		...
		if ($selected_items != false) {
			...
			if (cacti_sizeof($template)) {
				...
				if (strlen($message)) {
					thold_raise_message(__('Created %s thresholds', $created,"thold") . '<br>' . $message, MESSAGE_LEVEL_INFO);
				} else {
